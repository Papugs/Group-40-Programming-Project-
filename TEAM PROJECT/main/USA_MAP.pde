public class USA_MAP {
    ...
    } else if (dowedexpand==2)
    {
      ...
      if (r != null && r.size() != 0 && show) {
        ...
        for (DataPoint flight : stateflight.datapointlist) {
          text(Stateabb[indexButton], 600, 110);
          text("FL_DATE", 30, 130);
          text("DEST", 912, 130);
          text("DEST(ABR)", 711, 130);
          text("ORIGIN", 282, 130);
          text("ORIGIN(ABR)", 519, 130);
          ...
          if (110+offsets[i]+mouse > 140) {
            text(flight.getData(0), 30, 130+offsets[i]+mouse);
            text(flight.getData(4), 282, 130+offsets[i]+mouse);
            text(flight.getData(5), 912, 130+offsets[i]+mouse);
            text(flight.getData(8), 711, 130+offsets[i]+mouse);
            text(flight.getData(9), 519, 130+offsets[i]+mouse);
          }
          i++;
        }
        first = false;
  ...
  void expandButton() {
    ...
    if (buttonlist.get(indexButton).y > 90) buttonlist.get(indexButton).y -= 20;
